...
package ai.progenius.emefa.channel.wechat

import ai.progenius.emefa.channel.Channel
import ai.progenius.emefa.channel.ChannelHandler
...
class WeChatChannelHandler(
    ...
    private fun processInboundMessage(msg: WeChatMessage) {
        ...
        if (body.isEmpty()) {
            ...
            val app = ai.progenius.emefa.EmefaApplication.instance
            val mediaTypes = msg.itemList?.filter { WeChatInbound.isMediaItem(it) }?.map {
                when (it.type) {
                    MessageItemType.IMAGE -> app.getString(ai.progenius.emefa.R.string.wechat_media_image)
                    MessageItemType.VIDEO -> app.getString(ai.progenius.emefa.R.string.wechat_media_video)
                    MessageItemType.FILE -> app.getString(ai.progenius.emefa.R.string.wechat_media_file)
                    MessageItemType.VOICE -> app.getString(ai.progenius.emefa.R.string.wechat_media_voice)
                    else -> app.getString(ai.progenius.emefa.R.string.wechat_media_unknown)
                }
            } ?: emptyList()
            if (mediaTypes.isNotEmpty()) {
                XLog.i(TAG, "Pure media message received: types=$mediaTypes, from=${fromUserId.takeLast(16)}")
                val tip = app.getString(ai.progenius.emefa.R.string.wechat_unsupported_media, mediaTypes.joinToString("+"))
