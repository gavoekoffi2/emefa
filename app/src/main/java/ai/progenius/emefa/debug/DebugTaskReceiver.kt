 ...
 *
 * Usage:
 *   adb shell am broadcast -a ai.progenius.emefa.DEBUG_TASK --es task "open my camera"
 *
 * Set Cloud LLM config (any provider):
 *   adb shell am broadcast -a ai.progenius.emefa.DEBUG_TASK --es task "config:" \
...
class DebugTaskReceiver : BroadcastReceiver() {
            ...
            try {
                ...
                if (provider == "LOCAL") {
                    ...
                    val resolvedBaseUrl = baseUrl
                        ?: ai.progenius.emefa.agent.CloudProvider.findProviderForModel(modelName ?: "")?.defaultBaseUrl
                ...
                val vm = ai.progenius.emefa.EmefaApplication.appViewModelInstance
    ...
    private fun executeTool(intent: Intent, toolName: String, paramsJson: String) {
        try {
            ...
            breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "executeTool start tool=$toolName params=$params")
            ...
            if (result.isSuccess) {
                ...
                breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "executeTool success tool=$toolName data=${result.data}")
            } else {
                XLog.w("DebugTaskReceiver", "Debug tool failed: $toolName -> ${result.error}")
                breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "executeTool fail tool=$toolName error=${result.error}")
            }
        } catch (e: Exception) {
            XLog.e("DebugTaskReceiver", "Failed to execute debug tool", e)
            breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "executeTool exception tool=$toolName error=${e.message}")
        }
    }
    ...
    private fun simulateIncomingMessage(intent: Intent, simulateMessage: String) {
        try {
            ...
            breadcrumb(
                ai.progenius.emefa.EmefaApplication.instance,
        ...
        try {
            when (action.lowercase()) {
                "status" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend status ${LocalBackendHealth.debugStateSummary()}")
                }
                "force_cpu_safe" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend force_cpu_safe reason=$reason ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_cpu_safe" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend clear_cpu_safe ${LocalBackendHealth.debugStateSummary()}")
                }
                "mark_pending_gpu_init" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend mark_pending_gpu_init model=$modelPath ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_pending_gpu_init" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend clear_pending_gpu_init ${LocalBackendHealth.debugStateSummary()}")
                }
                "recover_pending_gpu_crash" -> {
                    ...
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend recover_pending_gpu_crash recovered=$recovered ${LocalBackendHealth.debugStateSummary()}")
                }
                else -> {
                    XLog.w("DebugTaskReceiver", "Unknown backend_action=$action")
                    breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend unknown action=$action")
                }
            }
        } catch (e: Exception) {
            XLog.e("DebugTaskReceiver", "Failed backend_action=$action", e)
            breadcrumb(ai.progenius.emefa.EmefaApplication.instance, "backend exception action=$action error=${e.message}")
        }
    }
