...
import ai.progenius.emefa.agent.langchain.LangChain4jToolBridge
import ai.progenius.emefa.agent.llm.LlmClient
import ai.progenius.emefa.agent.llm.LlmClientFactory
import ai.progenius.emefa.agent.llm.LlmResponse
import ai.progenius.emefa.agent.llm.StreamingListener
import ai.progenius.emefa.service.ClawAccessibilityService
import ai.progenius.emefa.tool.ToolRegistry
import ai.progenius.emefa.tool.impl.GetScreenInfoTool
...
- Do NOT auto-fill passwords, confirm payments, or delete data."""
            ...
            try {
                ...
                if (terminalCallback == null) {
                    if (cancelled.get()) {
                        ...
                        terminalCallback = {
                            callback.onComplete(0, EmefaApplication.instance.getString(R.string.agent_task_cancel), 0)
                        }
                    } else {
    ...
    private fun preCheck(): String? {
        if (ClawAccessibilityService.getInstance() == null) {
            return EmefaApplication.instance.getString(R.string.agent_accessibility_not_enabled)
        }
        return null
    }
    ...
    private fun chatWithRetry(messages: List<ChatMessage>, callback: AgentCallback, iteration: Int): LlmResponse {
        ...
        for (attempt in 0 until MAX_API_RETRIES) {
            if (cancelled.get()) throw RuntimeException(EmefaApplication.instance.getString(R.string.agent_task_cancelled))
            try {
    ...
    private fun runAgentLoop(userPrompt: String, callback: AgentCallback) {
        ...
        while (iterations < maxIterations && !cancelled.get()) {
            ...
            try {
                ...
                callback.onError(iterations, RuntimeException(EmefaApplication.instance.getString(R.string.agent_api_call_failed, e.message)), totalTokens)
                return
            }

            if (cancelled.get()) {
                callback.onComplete(iterations, EmefaApplication.instance.getString(R.string.agent_task_cancel), totalTokens, actualModelName)
                return
            }
            ...
            if (!llmResponse.hasToolExecutionRequests()) {
                ...
                callback.onComplete(iterations, EmefaApplication.instance.getString(R.string.agent_task_completed), totalTokens, actualModelName)
                continue
            }
            ...
            for (toolRequest in llmResponse.toolExecutionRequests) {
                ...
                if (toolName == "finish" && result.isSuccess) {
                    ...
                    callback.onComplete(iterations, finishData ?: EmefaApplication.instance.getString(R.string.agent_task_completed), totalTokens, actualModelName)
                    return
                }
        ...
        if (cancelled.get()) {
            ...
            callback.onError(iterations, RuntimeException(EmefaApplication.instance.getString(R.string.agent_max_iterations, maxIterations)), totalTokens)
        }
    }
