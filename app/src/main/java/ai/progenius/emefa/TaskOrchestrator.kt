...
import ai.progenius.emefa.agent.AgentCallback
import ai.progenius.emefa.agent.AgentConfig
import ai.progenius.emefa.agent.AgentService
import ai.progenius.emefa.agent.AgentServiceFactory
import ai.progenius.emefa.agent.PipelineRouter
import ai.progenius.emefa.agent.skill.SkillExecutor
import ai.progenius.emefa.agent.skill.SkillRegistry
...
class TaskOrchestrator(
    ...
    private val pipelineRouter = PipelineRouter(EmefaApplication.instance)
    ...
    fun cancelCurrentTask() {
        ...
        if (ForegroundService.isRunning()) {
            ForegroundService.updateTaskStatus(EmefaApplication.instance, "Stopping task...")
        }
    ...
    fun startNewTask(
                    ...
                    if (skill != null) {
                        ...
                        Thread({
                            val skillResult = skillExecutor.execute(skill, route.params) { step, total, desc ->
                                ...
                                ForegroundService.updateTaskStatus(EmefaApplication.instance, desc)
                            }
        ...
        if (!::agentService.isInitialized) {
            ...
            try {
                ...
                ChannelManager.sendMessage(channel, EmefaApplication.instance.getString(R.string.channel_msg_service_not_ready), messageID)
                return
            }
        }
        ...
        agentService.executeTask(agentPrompt, object : AgentCallback {
            override fun onLoopStart(round: Int) {
                ...
                if (round > 1) {
                    ...
                    if (ForegroundService.isRunning()) {
                        ForegroundService.updateTaskStatus(EmefaApplication.instance, "Step $round")
                    }
                }
            }

            override fun onTokenUpdate(status: ai.progenius.emefa.agent.TokenMonitor.Status) {
            ...
            override fun onToolCall(round: Int, toolId: String, toolName: String, parameters: String) {
                ...
                if (!floatingShown && !isFinish) {
                    ...
                    FloatingCircleManager.ensureShowing()
                    FloatingCircleManager.showTaskNotify(task, channel)
                    ForegroundService.updateTaskStatus(EmefaApplication.instance, "Running task...")
                }
                if (toolName.isNotEmpty()) {
                    val displayName = ai.progenius.emefa.tool.ToolRegistry.getInstance().getDisplayName(toolName)
                    taskEventCallback?.invoke(TaskEvent.ToolAction(displayName))
                    ForegroundService.updateTaskStatus(EmefaApplication.instance, "$displayName...")
                }
            }

            override fun onToolResult(round: Int, toolId: String, toolName: String, parameters: String, result: ToolResult) {
                val app = EmefaApplication.instance
            ...
            override fun onComplete(round: Int, finalAnswer: String, totalTokens: Int, modelName: String?) {
                ...
                val cancelAnswers = setOf(
                    EmefaApplication.instance.getString(R.string.agent_task_cancel),
                    EmefaApplication.instance.getString(R.string.agent_task_cancelled),
                    EmefaApplication.instance.getString(R.string.channel_msg_task_cancelled)
                )
                ...
                FloatingCircleManager.setSuccessState()
                // Auto-return to EMEFA after in-app task completes
                if (completedSession.autoReturnToChat) {
                    XLog.i(TAG, "onComplete: auto-returning to EMEFA chatroom")
                    try {
            ...
            override fun onError(round: Int, error: Exception, totalTokens: Int) {
                ...
                ChannelManager.sendMessage(
                    failedChannel,
                    EmefaApplication.instance.getString(R.string.channel_msg_task_error, error.message),
                    failedMessageId
                )
            ...
            override fun onSystemDialogBlocked(round: Int, totalTokens: Int) {
                ...
                ChannelManager.sendMessage(
                    blockedChannel,
                    EmefaApplication.instance.getString(R.string.channel_msg_system_dialog_blocked),
                    blockedMessageId
                )
                try {
