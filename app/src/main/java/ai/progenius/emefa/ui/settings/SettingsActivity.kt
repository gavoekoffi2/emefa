...
import ai.progenius.emefa.widget.AlertDialog
import ai.progenius.emefa.widget.CommonToolbar
import ai.progenius.emefa.widget.MenuGroup
import ai.progenius.emefa.widget.MenuItem
import ai.progenius.emefa.AppCapabilityCoordinator
import ai.progenius.emefa.AppRequirement
...
class SettingsActivity : BaseActivity() {
    ...
    private var permAccessibility: ai.progenius.emefa.widget.MenuItem? = null
    private var permNotification: ai.progenius.emefa.widget.MenuItem? = null
    private var permNotifAccess: ai.progenius.emefa.widget.MenuItem? = null
    private var permOverlay: ai.progenius.emefa.widget.MenuItem? = null
    private var permBattery: ai.progenius.emefa.widget.MenuItem? = null
    private var permStorage: ai.progenius.emefa.widget.MenuItem? = null
    ...
    private fun initMenuGroups() {
        ...
        modelGroup.addMenuItem(
            ...
            setTrailingText(ai.progenius.emefa.agent.TaskBudget.describeCurrentBudget())
        }
        ...
        lifecycleScope.launch {
            ...
            runCatching {
                ...
                val intent = Intent(Intent.ACTION_SEND).apply {
                    ...
                    putExtra(Intent.EXTRA_SUBJECT, "EMEFA debug report ${ai.progenius.emefa.BuildConfig.VERSION_NAME}")
                    putExtra(Intent.EXTRA_TEXT, "Attach this debug report when reporting a EMEFA issue.")
    ...
    private fun showBudgetDialog() {
        val currentTokens = ai.progenius.emefa.agent.TaskBudget.getConfiguredMaxTokens()
        val currentCost = ai.progenius.emefa.agent.TaskBudget.getConfiguredMaxCost()
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                when (newTokens) {
                    null -> ai.progenius.emefa.agent.TaskBudget.clearMaxTokens()
                    else -> ai.progenius.emefa.agent.TaskBudget.saveMaxTokens(newTokens)
                }
                when {
                    newCost == null || newCost <= 0.0 -> ai.progenius.emefa.agent.TaskBudget.clearMaxCost()
                    else -> ai.progenius.emefa.agent.TaskBudget.saveMaxCost(newCost)
                }

                val summary = ai.progenius.emefa.agent.TaskBudget.describeCurrentBudget()
