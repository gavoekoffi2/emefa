...
import ai.progenius.emefa.agent.llm.ActiveModelMode
...
import ai.progenius.emefa.widget.QRCodeDialog
...
class SettingsViewModel : ViewModel() {
    ...
    fun refresh() {
        ...
        val map = mapOf(
            ...
            MenuAction.DISCORD.name to SettingValue.Text(EmefaApplication.instance.getString(if (discordBotToken) R.string.common_bound else R.string.common_unbound)),
            MenuAction.TELEGRAM.name to SettingValue.Text(EmefaApplication.instance.getString(if (telegramBotToken) R.string.common_bound else R.string.common_unbound)),
            MenuAction.WECHAT.name to SettingValue.Text(EmefaApplication.instance.getString(if (wechatBotToken) R.string.common_bound else R.string.common_unbound)),
    ...
    fun startWeChatQrLogin(context: Context) {
        viewModelScope.launch {
            val loadingDialog = ai.progenius.emefa.widget.LoadingDialog.show(
                context = context,
                message = context.getString(R.string.channel_config_wechat_scanning)
            )
            try {
                val apiClient = ai.progenius.emefa.channel.wechat.WeChatApiClient()
    ...
    private fun startWeChatQrPolling(
        context: Context,
        dialog: QRCodeDialog,
        apiClient: ai.progenius.emefa.channel.wechat.WeChatApiClient,
        qrcode: String
    ): Job {
                ...
                try {
                    ...
                    if (authResult != null) {
                        ...
                        dialog.showStatusOverlay(
                            EmefaApplication.instance.getString(R.string.channel_config_wechat_confirmed)
                        )
                        refresh()
                        delay(1500)
                        dialog.dismiss()
    ...
    fun toggleConfigServer(context: Context): String {
        return if (ConfigServerManager.isRunning()) {
            ...
            if (started) {
                ...
                EmefaApplication.instance.getString(R.string.lan_config_no_wifi)
            }
        }
    }
    ...
    private fun getLanConfigTrailingText(): String {
        return if (ConfigServerManager.isRunning()) {
            ConfigServerManager.getAddress() ?: EmefaApplication.instance.getString(R.string.lan_config_stopped)
        } else {
            EmefaApplication.instance.getString(R.string.lan_config_stopped)
        }
    }
    ...
    fun unbindWeChat() {
        ...
        ai.progenius.emefa.channel.wechat.WeChatInbound.clearContextTokensForAccount(accountId)
